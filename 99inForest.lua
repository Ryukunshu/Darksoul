...
if game.PlaceId == 79546208627805 then
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Darksoul | 99 Nights In The Forest",
            Text = "Go In Game for Darksoul to load :D [You are in lobby currently]",
            Duration = 10
        })
    end)
    return
                    ...
                    if TextSource then
                        ...
                        if plr then
                            ...
                            if plr:GetAttribute("__OwnsVIPGamepass") and plr:GetAttribute("VIPChatTag") ~= false then
                                prefix = prefix .. "<font color='rgb(200,0,0)'>[VIP]</font> "
                            end
                            local currentLevel = plr:GetAttribute("_CurrentLevel")
                            if currentLevel then
                                prefix = prefix .. string.format("<font color='rgb(200,0,0)'>[</font><font color='rgb(255,255,255)'>%s</font><font color='rgb(200,0,0)'>]</font> ", tostring(currentLevel))
                            end
                            local playerTagValue = plr:FindFirstChild("PlayerTagValue")
                            if playerTagValue and playerTagValue.Value then
                                prefix = prefix .. string.format("<font color='rgb(200,0,0)'>[</font><font color='rgb(255,255,255)'>#%s</font><font color='rgb(200,0,0)'>]</font> ", tostring(playerTagValue.Value))
                            end
...
-- swapped to Darksoul repo path
loadstring(game:HttpGet("https://raw.githubusercontent.com/Ryukunshu/Darksoul/"..tostring(commit).."/newnightsintheforest.lua", true))()                        local plr = Players:GetPlayerByUserId(TextSource.UserId)    
                        if plr then    
                            local prefix = ""    
                            if TagRegister[plr] then    
                                prefix = prefix .. TagRegister[plr]    
                            ...
                            if plr:GetAttribute("__OwnsVIPGamepass") and plr:GetAttribute("VIPChatTag") ~= false then    
                                prefix = prefix .. "<font color='rgb(255,0,0)'>[VIP]</font> "    
                            end    
                            local currentLevel = plr:GetAttribute("_CurrentLevel")    
                            if currentLevel then    
                                prefix = prefix .. string.format("<font color='rgb(0,0,0)'>[</font><font color='rgb(255,0,0)'>%s</font><font color='rgb(0,0,0)'>]</font> ", tostring(currentLevel))    
                            end    
                            local playerTagValue = plr:FindFirstChild("PlayerTagValue")    
                            if playerTagValue and playerTagValue.Value then    
                                prefix = prefix .. string.format("<font color='rgb(0,0,0)'>[</font><font color='rgb(255,0,0)'>#%s</font><font color='rgb(0,0,0)'>]</font> ", tostring(playerTagValue.Value))    
                            end    
                            prefix = prefix .. PrefixText    
                            properties.PrefixText = string.format("<font color='rgb(255,0,0)'>%s</font>", prefix)    
                        end    
                    end    
                    return properties    
                end    
